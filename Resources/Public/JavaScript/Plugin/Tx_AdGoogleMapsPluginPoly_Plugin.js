...
Tx_AdGoogleMaps_Plugin.prototype.drawPolyline = function(layerIndex){
	...
	// Set layer object.
	if (layerOptions.options.path !== null) {
		layerOptions.options.map = this.map;
		var layer = new google.maps.Polyline(layerOptions.options);
		this.setLayer(layerUid, {
			options: layerOptions,
			layer: layer,
			bounds: this.getBoundsByLatLngArray(layer.getPath()),
			center: this.getBoundsByLatLngArray(layer.getPath())
		});
	}
...
Tx_AdGoogleMaps_Plugin.prototype.drawPolygon = function(layerIndex){
	...
	var layerUid = layerOptions.uid;
	// Set layer object.
	if (layerOptions.options.paths !== null) {
		layerOptions.options.map = this.map;
		var layer = new google.maps.Polygon(layerOptions.options);
		this.setLayer(layerUid, {
			options: layerOptions,
			layer: layer,
			bounds: this.getBoundsByLatLngArray(layer.getPath()),
